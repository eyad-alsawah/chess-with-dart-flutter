...
class GameStatus{
  ...
  bool isAnyKingAdjacentToAnotherKing() {
    for (var element in chessBoard) {
      if (element.piece == Pieces.king) {
        ...
        illegalMoves.getKingPieces(rank: element.rank, file: element.file);
  ...
  bool isCheckmate({required PlayingTurn attackedPlayer}) {
    ...
    for (var piece in attackedPlayerPieces) {
      ...
      illegalMoves.getIllegalAndLegalMoves(rank: piece.rank, file: piece.file);
      List<Square> legalMovesOnly = legalMoves.getLegalMovesOnly(
          legalAndIllegalMoves: legalAndIllegalMoves,
          file: piece.file,
          rank: piece.rank);
      List<int> legalMovesIndices = legalMoves.getLegalMovesIndices(
    ...
    List<int> kingLegalMovesIndices = legalMoves.getLegalMovesIndices(
  ...
  bool checkForStaleMate({required PieceType opponentKingType}) {
    ...
    for (var square in chessBoard) {
      if (square.pieceType == chessBoard[opponentKingIndex].pieceType) {
        allLegalMovesIndices.addAll(legalMoves.getLegalMovesIndices(
          tappedSquareFile: square.file,
          tappedSquareRank: square.rank,
          isKingChecked: ChessController.isKingInCheck,
